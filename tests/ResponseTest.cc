...
namespace Greenstack {
    ...
    TEST_F(ResponseTest, Constructor) {
        ...
        EXPECT_EQ(std::numeric_limits<uint16_t>::max(), response.getOpcode());
    ...
    TEST_F(ResponseTest, Opcode) {
        ...
        res.setOpcode(0xeeff);
        EXPECT_EQ(0xeeff, res.getOpcode());
        res.encode(data);
        EXPECT_EQ(0xee, data[4]);
        EXPECT_EQ(0xff, data[5]);
    }
    ...
    TEST_F(ResponseTest, Decode) {
        ...
        Message *created = Message::create(data);
