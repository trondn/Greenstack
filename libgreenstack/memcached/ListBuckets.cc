...
Greenstack::ListBucketsRequest::ListBucketsRequest() : Request(Opcodes::ListBuckets) {

}

Greenstack::ListBucketsResponse::ListBucketsResponse(const std::vector<std::string> &buckets) : Response(Opcodes::ListBuckets, Status::Success) {
...
Greenstack::ListBucketsResponse::ListBucketsResponse(Greenstack::status_t status) : Response(Opcodes::ListBuckets, status) {
...
void Greenstack::ListBucketsResponse::validate() {
    ...
    if (getStatus() == Status::Success) {
        ...
        if (!VerifyListBucketsResponseBuffer(verifier)) {
            throw std::runtime_error("Incorrect payload for ListBucketsResponse");
        }
    } else {
        if (payload.size() > 0) {
            throw std::runtime_error("Incorrect payload for ListBucketsResponse");
