...
std::string Greenstack::MutationType::to_string(Greenstack::mutation_type_t opcode) {
...
Greenstack::mutation_type_t Greenstack::MutationType::from_string(const std::string &val) {
...
Greenstack::MutationRequest::MutationRequest() : Request(Opcodes::Mutation) {
...
static Greenstack::Payload::mutation_type_t convertOperation(Greenstack::mutation_type_t operation) {
    switch (operation) {
        ...
        default:
            throw std::runtime_error(Greenstack::MutationType::to_string(operation));
    }
}
...
static Greenstack::compression_t convertCompression(Greenstack::Payload::compression_t compression) {
    switch (compression) {
        ...
        case Greenstack::Payload::compression_t_Bzip:
            return Greenstack::Compression::BZip;
        case Greenstack::Payload::compression_t_Gzip:
            return Greenstack::Compression::GZip;
        default:
            throw std::runtime_error("Received unknown compression");
    }
}

static Greenstack::datatype_t convertDatatype(Greenstack::Payload::datatype_t datatype) {
    switch (datatype) {
...
static Greenstack::mutation_type_t convertOperation(Greenstack::Payload::mutation_type_t operation) {
    switch (operation) {
...
Greenstack::MutationResponse::MutationResponse(cas_t cas, uint32_t size, uint64_t seqno, uint64_t vbucketuuid)
        : Response(Opcodes::Mutation, Status::Success) {
    flatbuffers::FlatBufferBuilder fbb;
...
Greenstack::MutationResponse::MutationResponse(Greenstack::status_t status) : Response(Opcodes::Mutation, status) {
    // empty
}

Greenstack::MutationResponse::MutationResponse() : Response(Opcodes::Mutation, Status::Success) {
    // empty
}
